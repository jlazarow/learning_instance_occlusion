...
class MaskPostProcessor(nn.Module):
    ...
    def forward(self, x, boxes, mask_scores=None):
        """
        Arguments:
        ...
        if not (mask_scores is None):
            mask_scores = torch.softmax(mask_scores, dim=1)
            mask_scores = torch.squeeze(mask_scores[index, labels][:, None], dim=1)
            mask_scores = mask_scores.split(boxes_per_image)
        else:
            mask_scores = len(boxes_per_image) * [None]

        results = []
        for prob, score, box in zip(mask_prob, mask_scores, boxes):
            bbox = BoxList(box.bbox, box.size, mode="xyxy")
            for field in box.fields():
                bbox.add_field(field, box.get_field(field))
            bbox.add_field("mask", prob)
            if not (score is None):
                bbox.add_field("rank", score)
